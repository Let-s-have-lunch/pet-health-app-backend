...
import replyService from "../service/replyService.ts";
...
const getRepliesByPostId = async (req: Request<{ postId: string }>, res: Response) => {
    try {
        ...
        if (isNaN(postId)) {
            res.status(401).json({ message: "유효하지 않은 게시글 ID 입니다." });
            return;
        }
        ...
        console.log(error);
        res.status(500).json({ message: "댓글 목록을 불러오는 중에 오류가 발생했습니다." });
    }
};
...
const createReply = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(201).send({ message: "댓글이 성공적으로 작성되었습니다.", data: result });
    } catch (error) {
...
export default { createReply, getRepliesByPostId, deleteReply, updateReply };
